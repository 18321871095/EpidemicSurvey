...
$("#tijiao15").click(function () {
    var data={},message=[];
try{

    for(var i=1;i<=31;i++){
        data["qacem"+i]=$("[name=qacem"+i+"]:checked").val()==undefined ? "" : $("[name=qacem"+i+"]:checked").val();
    }
    for(var j=1;j<=31;j++){
        if(data["qacem"+j]==''){
            message.push(j);
    ...
    if(message.length==0){
        ...
        sendRequest("提交中...",10000,"/survey/addAnswer",{data:JSON.stringify(data),num:15},function (data) {
            if(data.msg=='success'){
