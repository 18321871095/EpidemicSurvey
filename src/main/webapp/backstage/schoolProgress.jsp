...
<script>
    ...
    $(function () {
        ...
        layui.use(['element', 'layer'], function () {
            ...
            $.post("/backstage/selProgressBySchoolId",{type:'1'},function (data) {
                layer.close(index);
                if(data.msg=='success'){
                    $("#mytbody").empty();
                    var complete=0;
                    for(var i=0;i<data.result.length;i++){
                            if(state=='1'){
                                $("#mytbody").append("<tr>\n" +
                                    ...
                                    "            <td>"+getAgree(data.result[i].agree)+"</td>\n" +
                                    "            <td>\n" +
                                    "                <div class=\"layui-progress layui-progress-big\" lay-showPercent=\"yes\">\n" +
                                    "                    <div class=\"layui-progress-bar layui-bg-green\" lay-percent="+getProgress(data.result[i].state,data.result[i].currentNum)+"></div>\n" +
                                    "                </div>\n" +
                                    "            </td>\n" +
                                    "        </tr>");
                            }else{
                        ...
                        for(var i=0;i<noCompleteList.length;i++){
                            if(state=='1'){
                                $("#mytbody1").append("<tr>\n" +
                                    ...
                                    "            <td>\n" +
                                    "                <div class=\"layui-progress layui-progress-big\" lay-showPercent=\"yes\">\n" +
                                    "                    <div class=\"layui-progress-bar layui-bg-green\" lay-percent="+getProgress(noCompleteList[i].state,noCompleteList[i].currentNum)+"></div>\n" +
                                    "                </div>\n" +
                                    "            </td>\n" +
                                    "        </tr>");
                            }else{
    ...
    function getProgress(state,currentNum) {
        try {
            if (state == '1') {
                return "100%";
            } else {
