...
<div style="padding: 10px 10px">
    ...
    <button style="width: 12%;" id="xaizaiinfo"
            class="layui-btn layui-btn-normal layui-btn-sm "><i class="layui-icon">&#xe601;</i>下载大班名单</button>
    <c:if test="${sessionScope.LoginInfo.getAdminPss()==1}">
        <button style="width: 12%;" id="download" class="layui-btn layui-btn-sm">
            <i class="layui-icon">&#xe601;</i>下载大班问卷码</button>
        <button style="width: 12%;" id="download_file" class="layui-btn layui-btn-sm">
            <i class="layui-icon">&#xe601;</i>下载大班调研说明</button>
    </c:if>
<c:if test="${sessionScope.LoginInfo.getAdminPss()!=1}">
    ...
    <span style="font-size: 20px;color: red;float: right;">大班在园人数：<span id="zaiyuan"></span></span>
</div>
<%--565--%>
       ...
       // console.log(document.body.clientHeight)// 554 621     722  810  1.12
        ...
        layui.use(['layer','form','table'], function(){
            ...
            $("#tijiao").click(function () {
                layer.confirm('确认提交？提交之后就无法再次修改保存了。', {icon: 3, title:'提示',offset: '200px'}, function(index){
                    ...
                    $.ajax({
                        ...
                        timeout:15000,
                        data:{data1:data_1_tijiao,data2:data_2_tijiao,data3:data_3_tijiao,data4:data_4_tijiao,data5:data_5_tijiao,type:1,type1:2},
                        success: function (data) {
                           layer.close(index1);
                            layer.close(index);
                            if(data.msg=='success'){
            ...
            $("#baocun").click(function () {
                layer.confirm('确认保存？', {icon: 3, title:'提示',offset: '200px'}, function(index){
                   ...
                   $.ajax({
                        ...
                        url: "/backstage/confirmStudent",
                        timeout:15000,
                        data:{data1:data_1,data2:data_2,data3:data_3,data4:data_4,data5:data_5,type:0,type1:2},
                        success: function (data) {
                            layer.close(index1);
                            layer.close(index);
                            if(data.msg=='success'){
            ...
            $("#download").click(function () {
                ...
                $.post("/backstage/downloadToken",{type:2},function (data) {
                    layer.close(index);
                    if(data.msg=='success'){
                        window.location.href="/backstage/downloadToken1?type=2";
                    }else {
                        layer.alert(data.result,{offset:'200px'});
                    }
                });
            ...
            //下载学生名单
            $("#xaizaiinfo").click(function () {
                var index = layer.load(2,{offset:'200px'});
                $.post("/backstage/downloadStudetInfo",{type:2},function (data) {
                    layer.close(index);
                    if(data.msg=='success'){
                        window.location.href="/backstage/downloadStudetInfo1?type=2";
                    }else {
                        layer.alert(data.result,{offset:'200px'});
                    }
                });
            ...
            $("#download_file").click(function () {
               window.location.href="/backstage/downloadFiles?type=2";
            });
